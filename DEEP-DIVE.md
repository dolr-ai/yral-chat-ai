...
When someone opens `https://chat-ai.rishi.yral.com/`
in their browser, here's EXACTLY what happens, step by step:

```
1. Browser asks DNS: "what IP is chat-ai.rishi.yral.com?"
...
3. On the server, Caddy (a reverse proxy) receives the request
   → Caddy reads the Host header: "chat-ai.rishi.yral.com"
   → Caddy looks up its config: this domain maps to container "yral-chat-ai" on port 8000
...
- As the subdomain prefix (→ `chat-ai.rishi.yral.com`)
...
(e.g., `ghcr.io/dolr-ai/yral-chat-ai`). `${IMAGE_TAG}`
...
```
    USER'S BROWSER
         |
         | 1. "What IP is chat-ai.rishi.yral.com?"
         v
    CLOUDFLARE DNS
         |
...
`chat-ai-db-internal`). This means:
...
3. Runs `init-from-template.sh` which edits ONLY project.config
   (replaces `chat-ai` with `my-service` everywhere)
