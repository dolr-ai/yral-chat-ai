...
# (e.g., /home/deploy/chat-ai-db-stack/)
cd "${STACK_DIR}"
...
# ----- Compute content sha for haproxy.cfg (future-proof against Swarm config immutability) -----
...
# This exact error broke the TEMPLATE's deploys from 2026-04-12 through
# 2026-04-20; yral-chat-ai's haproxy.cfg hadn't drifted yet so it was fine,
# but it would hit the same trap on the next change. The template's
# deploy-db-stack.sh has the same guard.
#
# Fix: version the Swarm config name by a content sha so every content
# change creates a NEW config. haproxy/stack.yml references
# `${HAPROXY_CFG_SHA}`; the old config is orphaned (no one references it)
# and we prune it below.
#
...
#
# "${SWARM_STACK}" is the stack name (e.g., "chat-ai-db")
docker stack deploy \
    --with-registry-auth \
    -c etcd-stack.yml \
    -c patroni-stack.yml \
